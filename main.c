...
Book* searchBook(void) {
	...
	printf("\t-------Перед началом поиска требуется определить, по какому полю в информации о книге искать--------\n"
		...
		"\t| 3. Искать по году выпуска книги                                                                  |\n"
	...
	* с книгами в дереве для поиска книг с аналогичным полем */
	...
	case SEARCH_BY_YEAR: {
		searchBook->issueYear = (int)getUserNumber("Введите год выпуска книги для поиска (целое число, если до нашей эры - вводить со знаком минус) > ");
		break;
	}
	default:
...
DWORD editBook(void) {
	...
	printf("\t----------------------------------------------------------------------------------------------------\n"
		...
		"\t| 3. Отредактировать год выпуска книги                                                             |\n"
