...
// Делаем информацию о всех деревьях доступной во всех файлах
extern treeNode* trees[7];
...
/*Функция, с помощью которой можно найти определенную книгу в библиотеке по любому из полей (кроме краткого описания).
Возвращает указатель на конкретную книгу, найденную (и, если по указанному параметру нашлась не одна книга, впоследствии выбранную) 
пользователем. */
...
/*Функция считывает одну строку из файла (до символа переноса строки или до EOF), выделяет под нее нужное количество памяти из кучи,
сохраняет туда эту строку и возвращает указатель на начало строки. */
...
// Загружает книги из базы данных в текущие деревья в программе, всем загруженным книгам дает новые id, используя основной счетчик
